...
vkint.command('/addchat', (ctx) => {
  ...
  connection.query(`SELECT * FROM \`mods\` WHERE \`vkid\` = '${ctx.message.from_id}' AND \`active\` = '1'`, async (error, result, packets) => {
    ...
    connection.query(`INSERT INTO \`chats\`(\`server\`,\`cid\`,\`name\`,\`mlvl\`) VALUES ('${args[1]}', '${chat}', '${name}', '${args[2]}')`);
...
vkint.command('/mwarn', (ctx) => {
  ...
  connection.query(`SELECT * FROM \`mods\` WHERE \`vkid\` = '${ctx.message.from_id}' AND \`active\` = '1'`, async (error, result, packets) => {
    ...
    if(result[0].server == 9 && result[0].mlvl < 4 && result[0].fulldostup == 0) return ctx.reply(`Данная команда на сервере Yuma доступно с заместителя гл.модератора`);
    let text = ctx.message.text;  
